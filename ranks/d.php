<?php
  ...
  if(auth::hasAccess()) {
    ...
    if(net::get('a') != '') {
      switch (net::get('a')) {
        case 'enable':
          $stmt= $db->select('PLUGIN_RANKS', 'ID = '.net::get('i'));
          if($stmt->rowCount() > 0) {
            $user = $stmt->fetch();
        ...
        case 'add':
          ...
          echo '<div class=\'form-group\'>';
          echo '<label class=\'col-sm-2\' for=\'value\'>Experience</label>';
          echo '<div class=\'col-sm-10\'><input class=\'form-control\' data-fv-notempty data-fv-numeric id=\'experience\' name=\'experience\' placeholder=\'Experience\' type=\'text\' value=\'0\'></div>';
          echo '</div>';
        ...
        case 'edit':
          ...
          if($stmt->rowCount() > 0) {
            ...
            echo '<input class=\'form-control\' id=\'experience\' data-fv-notempty data-fv-numeric name=\'experience\' placeholder=\'Experience\' type=\'text\' value=\''.$user['EXPERIENCE'].'\'>';
            echo '</div>';
            echo '</div>';
