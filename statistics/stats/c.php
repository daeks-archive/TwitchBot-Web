    ...
    if(net::get('a') != '') {
      switch (net::get('a')) {
        case 'init':
          ...
          $stmt = $db->select('PLUGIN_STATS', $where.$limit);
          $data = array();
          foreach($stmt->fetchAll() as $row) {
            array_push($data, $row);
          }
          net::data($db->query('SELECT COUNT(*) AS TOTAL FROM PLUGIN_STATS WHERE '.$where)->fetch()['TOTAL'], json_encode($data));
        break;
        case 'enable':
          if(net::get('i') != '') {
            $db->update('PLUGIN_STATS', array('ENABLED' => 1, 'UPDATEBY' => auth::getUserName()), 'ID='.net::get('i'));
        ...
        case 'disable':
          if(net::get('i') != '') {
            $db->update('PLUGIN_STATS', array('ENABLED' => 0, 'UPDATEBY' => auth::getUserName()), 'ID='.net::get('i'));
        ...
        case 'edit':
          if(net::get('i') != '') {
            ...
            $db->update('PLUGIN_STATS', $fields, 'ID='.net::get('i'));
        ...
        case 'del':
          if(net::get('i') != '') {
            $db->delete('PLUGIN_STATS', 'ID='.net::get('i'));
