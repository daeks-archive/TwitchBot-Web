<?php
  ...
  if(auth::hasAccess()) {
    ...
    if(net::get('a') != '') {
      switch (net::get('a')) {
        case 'enable':
          $stmt= $db->select('PLUGIN_LOYALITY', 'ID = '.net::get('i'));
          if($stmt->rowCount() > 0) {
            $user = $stmt->fetch();
        ...
        case 'add':
          ...
          echo '<div class=\'form-group\'>';
          echo '<label class=\'col-sm-2\' for=\'value\'>Loyality Points</label>';
          echo '<div class=\'col-sm-10\'><input class=\'form-control\' data-fv-notempty data-fv-numeric id=\'value\' name=\'value\' placeholder=\'Loyality Points\' type=\'text\' value=\'\'></div>';
          echo '</div>';
        ...
        case 'edit':
          ...
          if($stmt->rowCount() > 0) {
            ...
            echo '<input class=\'form-control\' id=\'value\' data-fv-notempty data-fv-numeric name=\'value\' placeholder=\'Loyality Points\' type=\'text\' value=\''.$user['VALUE'].'\'>';
            echo '</div>';
            echo '</div>';
