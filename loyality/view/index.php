      ...
      if(sizeof($query) == 2) {
        ...
        if($stmt2->rowCount() > 0) {
          ...
          if($stmt3->rowCount() > 0) {
            ...
            $stmt4 = $db->query('SELECT * FROM plugin_loyality WHERE BOTID='.$bot['ID'].' and CHANNELID='.$channel['ID'].' and enabled=1 and value <> 0 order by value DESC limit 0, '.MAX);
            if($stmt4->rowCount() > 0) {
              $i = 1;
            ...
            echo '</div>';
            echo '<p>Loyality points are given by the broadcaster to indicate a specific channel award. Normally these points are used for games or bets.</p>';
            echo '</div>';
