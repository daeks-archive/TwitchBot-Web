      ...
      if(sizeof($query) == 2) {
        ...
        if($stmt2->rowCount() > 0) {
          ...
          if($stmt3->rowCount() > 0) {
            ...
            $stmt4 = $db->query('SELECT * FROM plugin_quotes WHERE BOTID='.$bot['ID'].' and CHANNELID='.$channel['ID'].' order by ID desc');
            if($stmt4->rowCount() > 0) {
              $i = 1;
              foreach($stmt4->fetchAll() as $quote) {        
                echo '<div class="card">';
                echo '<div style="flex-basis: 70px;" class="card-block card-ranking-block">';
                echo '<h6 class="text-muted">Quote</h6><h3>#'.$quote['ID'].'</h3>';
                echo '</div>';
                echo '<div style="flex-basis: 800px;" class="card-block">';
                echo '<div class="card-user">';
                echo '<div class="card-user-name">';
                echo '<i class="fa fa-quote-left"></i> '.$quote['VALUE'].' <i class="fa fa-quote-right"></i>';
                echo '</div>';
                echo '</div>';
                echo '</div>';
                echo '<div style="flex-basis: 150px;" class="card-block align-right">';
                echo '<h6 class="text-muted">'.date('d.m.Y H:i', strtotime($quote['INSERTED'])).'</h6><h3></h3>';
                echo '</div>';
                echo '</div>';
                $i++;
              }
            ...
            echo '</div>';
            echo '<p>Quotes are funny messages from the broadcaster on the channel.</p>';
            echo '</div>';
